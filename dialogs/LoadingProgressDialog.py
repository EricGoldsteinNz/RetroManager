...
class LoadingProgressDialog(QMessageBox):
    ...
    def showProgress(self, progressValue, refreshBoolean=True):
        self.progress.setValue(progressValue)
        # This really is tough on long calls on performance, let's only do it when needed
        if refreshBoolean:
